...
void loadMarbleTexture() {
	// Load the marble texture from the .raw file
	if (!loadRawTexture("m02_marble.raw", 512, 512, 3)) {
		cout << "Failed to load marble texture!" << endl;
		return;
	}

	// Wrap modes for S and T based on exer
	switch (exer) {
	case 1:  // Repeat both S and T
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
		break;
	case 2:  // Clamp to edge both S and T
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
		break;
	case 3:  // Repeat S, Clamp T
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
		break;
	case 4:  // Clamp S, Repeat T
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
		break;
	}
...
setTextureFilter(int method)
{
	...
	if (method <= 2)
	{
		...
		case	6:	glMethod = GL_LINEAR_MIPMAP_LINEAR;		break;
...
init()
{
	...
	// Marble texture
	glBindTexture(GL_TEXTURE_2D, texID[2]);  // Assign marble texture to texID[2]
	loadMarbleTexture();
	...
	cout << "Keyboard input: x for axws on/off" << endl;
	...
	cout << "Keyboard input: up, down, right for viewing" << endl;
	cout << "Keyboard input: SHIFT + up, down for inc/decreasing # repeats" << endl;
	cout << endl;
	cout << "Keyboard input: g,h,j,k for exersise" << endl;
}
...
keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		case GLFW_KEY_G:
			exer = 1;
			glBindTexture(GL_TEXTURE_2D, texID[2]); // Marble texture ID
			loadMarbleTexture();
			break;
