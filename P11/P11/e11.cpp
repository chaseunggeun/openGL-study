...
vec3 phong(const vec3& n, const vec3& v, const Light& l, const vec3& r, const vec3& objectColor)
{
	vec3 I = ambient(l);
	...
	if (lambertian > 0)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			I[i] += objectColor[i] * m_diffuse[i] * lambertian * l.diffuse[i];
...
vec3 intensity(const Ray& ray, const Light l[], int nLights, int depth, int E = -1)
{
	...
	if (iObject != -1)
	{
		vec3 objectColor = objectColors[iObject];  // 구체의 색상
...
#pragma omp master
	...
	center_world[nSpheres] = vec3(-d, -d, 0); radius[nSpheres] = 0.7f; nSpheres++;
	center_world[nSpheres] = vec3(-d, d, 0); radius[nSpheres] = 0.7f; nSpheres++;
	center_world[nSpheres] = vec3(d, d, 0); radius[nSpheres] = 0.7f; nSpheres++;
	center_world[nSpheres] = vec3(d, -d, 0); radius[nSpheres] = 0.7f; nSpheres++;
	center_world[nSpheres] = vec3(0, 0, r); radius[nSpheres] = 0.7f; nSpheres++;
	center_world[nSpheres] = vec3(0, 0, -r); radius[nSpheres] = 0.7f; nSpheres++;*/

	nLights = 0;

	vec3 largeSphereCenter = center_world[nSpheres - 1];  // 큰 구체의 위치
	float largeSphereRadius = radius[nSpheres - 1];        // 큰 구체의 반지름

	// 첫 번째 빛의 위치: 큰 구체 내부에 위치하도록 설정
	light[0].p = vec4(largeSphereCenter.x + largeSphereRadius * 0.5f, largeSphereCenter.y, largeSphereCenter.z, 0);
	light[0].ambient = vec3(1.0, 1.0, 1.0);
	light[0].diffuse = 0.5f * vec3(1.0, 1.0, 1.0);
	light[0].specular = vec3(1.0, 1.0, 1.0);
	nLights++;

	// 두 번째 빛의 위치: 큰 구체 내부에 위치하도록 설정
	light[1].p = vec4(largeSphereCenter.x - largeSphereRadius * 0.5f, largeSphereCenter.y, largeSphereCenter.z, 0);
	light[1].ambient = vec3(1.0, 1.0, 1.0);
	light[1].diffuse = 0.5f * vec3(1.0, 1.0, 1.0);
	light[1].specular = vec3(1.0, 1.0, 1.0);
	nLights++;
	...
	radius[nSpheres] = largeSphereRadius;
	nSpheres++;
