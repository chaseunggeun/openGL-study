...
init()
{
	...
	cout << "Keyboard Input: p for perspective/orthographic viiewing" << endl;
...
keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		else if (mods & GLFW_MOD_SHIFT)
		{
			...
			case GLFW_KEY_LEFT:
			{
				mat4 rotation = rotate(mat4(1.0f), rotationAngle, C.u);
				vec4 rotatedF = rotation * vec4(C.f, 0.0f);
				C.f = normalize(vec3(rotatedF));
				C.a = C.e + C.f;
				C.r = normalize(cross(C.f, C.u));
				break;
			}
