...
// Add new control variable after existing ones
bool sortCube = false; // Depth sorting for cube faces

// Add face data structure for cube depth sorting
struct CubeFaceData {
    int faceIndex;
    vec3 center;
    float depth;
...
// Function to sort cube faces
void sortCubeFaces(CubeFaceData* faces, const mat4& modelView) {
    // Calculate depths
    for (int i = 0; i < 6; i++) {
        vec4 center_eye = modelView * vec4(faces[i].center, 1.0f);
        faces[i].depth = -center_eye.z;
    }
...
drawTexturedCube()
{
    ...
    // Setup cube face data for sorting
    CubeFaceData faces[6];
    for (int i = 0; i < 6; i++) {
        faces[i].faceIndex = i;
        faces[i].center = (vertex[i][0] + vertex[i][1] + vertex[i][2] + vertex[i][3]) / 4.0f;
    }

    // Sort faces if enabled
    if (sortCube) {
        GLfloat modelView[16];
        glGetFloatv(GL_MODELVIEW_MATRIX, modelView);
        mat4 mvMatrix = make_mat4(modelView);
        sortCubeFaces(faces, mvMatrix);
    }

    // Draw faces in sorted/unsorted order
    for (int idx = 0; idx < 6; idx++) {
        int i = sortCube ? faces[idx].faceIndex : idx;
...
keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    if (action == GLFW_PRESS || action == GLFW_REPEAT)
    {
        ...
        case GLFW_KEY_S: sortCube = !sortCube; break;
        }
    }
}