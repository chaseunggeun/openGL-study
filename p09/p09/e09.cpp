...
int frame = 0;
GLfloat shininess = 100.0f;  // 초기 shininess 값
const GLfloat MAX_SHININESS = 128.0f;
const GLfloat MIN_SHININESS = 0.0f;
const GLfloat SHININESS_STEP = 5.0f;  // 한 번에 변경될 값
void
reinitialize()
{
	frame = 0;
...
init()
{
	...
	cout << "Keyboard Input: o for rotaion of objects" << endl;
...
drawArrow(const vec3& p, bool tailOnly)
{
	...
	float	arrawTailRadius = 0.05f;
	glColor3f(1, 0, 0);
	drawSphere(arrawTailRadius, 16, 16);
...
drawSpotLight(const vec3& p, float cutoff)
{
	...
	float	appexRadius = 0.06f * (0.5f + exponentValue / 128.0f);
	drawSphere(appexRadius, 16, 16);
...
keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		case GLFW_KEY_RIGHT:    // 오른쪽 화살표 키로 shininess 증가
			shininess = std::min(shininess + SHININESS_STEP, MAX_SHININESS);
			break;

		case GLFW_KEY_LEFT:     // 왼쪽 화살표 키로 shininess 감소
			shininess = std::max(shininess - SHININESS_STEP, MIN_SHININESS);
			break;
		}
	}
}
